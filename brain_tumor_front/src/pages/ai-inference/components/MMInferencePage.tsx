  ...
  const loadAllOcsData = async () => {
    try {
      ...
      const proteinList = confirmed
        .filter((item: any) => item.job_role === 'LIS' && item.job_type === 'PROTEIN')
        .map(mapOcsItem)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <h3 className="text-lg font-semibold text-gray-900 mb-3">환자 선택</h3>
        <select
