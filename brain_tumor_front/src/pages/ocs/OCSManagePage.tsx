...
export default function OCSManagePage() {
  ...
  // OCS 생성 페이지로 이동
  const handleCreateOCS = useCallback(() => {
    navigate('/ocs/create');
  }, [navigate]);
  ...
  return (
    <div className="page ocs-manage">
      {/* 헤더 */}
      <header className="page-header">
        <h2>OCS 관리</h2>
        <span className="subtitle">OCS를 조회하고 관리합니다</span>
      </header>
      ...
      <section className="filter-bar">
        <div className="filter-left">
          ...
          <button className="btn btn-primary" onClick={handleCreateOCS}>
            + OCS 생성
          </button>
        </div>
      ...
      <section className="content">
        {loading ? (
          <LoadingSpinner text="OCS 목록을 불러오는 중..." />
        ) : filteredOcsList.length === 0 ? (
          <EmptyState
            icon="📋"
            title="OCS가 없습니다"
            description="새 OCS를 생성하거나 필터 조건을 변경해주세요."
            action={{ label: 'OCS 생성', onClick: handleCreateOCS }}
          />
        ) : (
